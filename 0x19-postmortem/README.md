...
1. Checked running processes using `ps aux`. Two `apache2` processes - `root` and `www-da# Postmortem
...
3. In one terminntering a critical
...
replaces any `phpp` extensions in the file `/var/www/html/wal, ran `strace` on the PID of the `root` Apache process. In another, curled
...
In short, a typo. Gotta love'em. In full, the WordPress app was encoup-settings.php` with `php`.
