...
### [0. How many subs?](./0-subs.py)# 0x16. API advanced
...
[200~* Write a function that queries the Reddit API and returns the number of subscribers (not active users, total subscribers) for a given subreddit. If an invalid subreddit is given, the function should return 0.
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        print("Ex: {} programming 'pythalid_subreddit 'python java'
wintermancer@lapbox ~/reddit_api/project $ 
```
---
on java javascript'".format(sys.argv[0]))
    else:
...
wintermancer@lapbox ~/reddit_api/project $ python3 100-main.py not_a_v
